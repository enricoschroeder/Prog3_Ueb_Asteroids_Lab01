...
werden Sie mithilfe von [raylib](https://www.raylib.com) bzw. [raylib-cpp](https://github.com/RobLoach/raylib-cpp) Ihr erstes grafisches Programm in C++ schreiben,
welches Ihren Spiel-Hintergrund anzeigt. Am Ende werden Sie Ihre erste C++-Klasse für das Raumschiff schreiben.
...
einzelnen Aufgaben ist genauer genannt, was ins Protokoll hinein muss.
...
Öffnen Sie CLion. Bestätigen Sie die Nutzerlizenz und wählen 'Start trial'. Sie sollten den Willkommensbildschirm
...
Bevor wir unsere komfortable Entwicklungsumgebung nutzen, wollen wir zuerst einmal ein kleines C++-Programm "per Hand"
kompilieren. Umso dankbarer werden Sie später für alles sein, was Ihnen Ihre Entwicklungsumgebung an Arbeit abnimmt!

Im Ordner `00_HelloMediaEngineers` finden Sie eine leere C++ Datei *main.cpp*. Implementieren Sie dort ein kleines
"Hello World" bzw. "Hello Media Engineers!" Programm, das einfach nur diesen Text auf der Kommandozeile ausgibt.

Kompilieren Sie dieses Programm **über das Terminal** und führen Sie es aus! Öffnen Sie dazu ein Terminal in CLion und
navigieren in den Ordner `00_HelloMediaEngineers` (mit dem Kommando `cd ` für "change directory"). Kompilieren Sie das
Programm nun über den entsprechenden Kommandozeilenbefehl und führen Sie es aus. Kompilieren Sie das Programm so, dass
das resultierende Programm "HelloMediaEngineers" heißt und aktivieren Sie sämtliche Kompilerwarnungen. Wenn Sie nicht
wissen, wie, recherchieren Sie im Skript oder mit einer Suchmaschine!

Notieren Sie die benötigten Schritte und ihr Kommando im Protokoll.
...
Wir verwenden [raylib](https://www.raylib.com) bzw. [raylib-cpp](https://github.com/RobLoach/raylib-cpp) für
Grafikausgabe. Dies ist eine s.g. *Dependency
...
Projektkonfiguration lädt Cmake raylib herunter und integriert es in Ihr Projekt. Sie bekommen davon im ersten Schritt
normalerweise nichts mit. Wir werden aber später noch etwas mit der Cmake-Konfiguration in Kontakt kommen.

#### Aufgabe 01_a

In CLion sollten in der oberen Symbolleiste ein paar Buttons vorfinden, so wie hier:

![](screenshots/screenshot_clion_1.png)

Stellen Sie sicher, dass das Projekt 01_Asteroids ausgewählt ist und drücken Sie nun auf den kleinen Hammer ('Build'),
um das leere Programm zu kompilieren. Nach einigen Sekunden sollten Sie eine Ausgabe wie diese sehen:
...
Glückwunsch, Sie haben Ihr erstes Programm mit CLion kompiliert. Sie können es mit dem Pfeil in der oberen Symbolleiste
starten. Allerdings werden Sie noch nicht viel sehen, da es aktuell noch leer ist. Sie werden es nun mit etwas Leben
füllen!
...
objektorientierten Code schreiben. Daher nutzen wir *raylib-cpp*, s.g. *C++ Bindings*. Diese machen die C Bibliothek
nativ aus C++ heraus nutzbar (mit Klassen und Vererbung und allem drum und dran). Die Dokumentation von raylib-cpp ist
nicht vollständig (da sie ja nur die Funktionen von raylib etwas "schöner" in C++ macht und man doppelte Dokumentation
...
Schreiben Sie Ihre main.cpp Datei so um, dass dort eine Instanz des Objekts erstellt wird und Methoden sinnvoll
aufgerufen werden, um es im Spielefenster anzuzeigen.
